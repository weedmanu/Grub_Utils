...
class TestContainerBasics(unittest.TestCase):
    ...
    def setUp(self):
        """Initialiser un conteneur pour chaque test."""
...
class TestContainerIntegration(unittest.TestCase):
    ...
    def test_realistic_grub_configuration(self):
        ...
        mock_grub_service = MagicMock(
            name='GrubService',
            backup_manager=mock_backup_mgr
        )
        mock_facade = MagicMock(
            name='GrubFacade',
            _service=mock_grub_service
        )
        mock_theme_mgr = MagicMock(name='ThemeManager')
