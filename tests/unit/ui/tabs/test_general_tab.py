...
class TestGeneralTab:
    def test_init(self):
        app = MagicMock()
        app.facade.entries = {"GRUB_DEFAULT": "0", "GRUB_TIMEOUT": "5", "GRUB_CMDLINE_LINUX_DEFAULT": "quiet splash"}
    ...
    def test_get_flat_menu_entries(self):
        ...
        app.facade.menu_entries = [{"id": "entry1", "title": "Entry 1"}, {"id": "entry2", "title": "Entry 2"}]
        ...
        with patch("src.ui.gtk_init.Gtk.DropDown.new_from_strings") as mock_new_dropdown:
            ...
            # Check entry_labels
            expected = ["Première entrée (0)", "Dernière utilisée (saved)", "Entry 1", "Entry 2"]
            assert tab.entry_labels == expected

    def test_init_custom_kernel_param(self):
        app = MagicMock()
        app.facade.entries = {"GRUB_CMDLINE_LINUX_DEFAULT": "custom param"}
        app.facade.get_menu_entries.return_value = []
